...
const StaffManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Switch to list tab to see the new staff member
      setActiveTab('list');
    } catch (err: any) {
    ...
    <MainLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">Manage hospital staff accounts and registrations</p>
            </div>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full max-w-md grid-cols-2 h-12 rounded-xl p-1">
            <TabsTrigger value="list" className="rounded-lg gap-2 text-sm font-medium">
              <List className="w-4 h-4" />
              Staff Directory
            </TabsTrigger>
            <TabsTrigger value="create" className="rounded-lg gap-2 text-sm font-medium">
